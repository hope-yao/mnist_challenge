...
**Update 2018-XX-XX:** Due to recent interest, we have added an L_2 option to our white-box challenge. The details of the challenge - including rules, submission format, and MNIST model - are provided below. In addition, we have set up a leaderboard to keep track of the best white-box attacks, which we plan to maintain in addition to the L_infinity leaderboards for the foreseeable future.

# (I'm not actually sure if it's better to put the L2 stuff before or after the Linf stuff - I chose before since it is more recent and likely more relevant to people that will visit this repo from here on out, but it feels weird to copy over a lot of the information from the Linf stuff)

## L_2 White-Box Leaderboard
...
## L_2 Format and Rules

The objective of this challenge is to find L_2 white-box attacks that are effective against our MNIST model.
Attacks are allowed to perturb each pixel of the input image by at most `epsilon=2.0`.

We invite any interested researchers to submit attacks against our model.
The most successful L_2 attacks will be listed in the leaderboard above.
...
The adversarial test set should be formated as a numpy array with each row containing a flattened array of 28x28 pixels corresponding to a single test example.
Thus, we expect the overall dimensions of your test set to be 10,000 rows and 784 columns.
Each pixel must be in the range [0,1].
See the script `pgd_attack.py` for an example of an attack that generates an adversarial test set in this format.

In order to submit your attack, save the matrix containing your adversarial examples with `numpy.save` and email the resulting file to mnist.challenge@gmail.com with the subject line "L2 Challenge Submission".
We will then run the `run_attack.py` script on your file to verify that the attack is valid and to evaluate the accuracy of our model on your examples.
...
## L_infinity Black-Box Leaderboard (Original Challenge)
...
## L_infinity White-Box Leaderboard
...
To ensure that the attacks are indeed black-box, we released our training code and model architecture, but keep the actual network weights secret. 

As of Oct. 15, 2017, we are no longer accepting black-box submissions. 
We are now interested in finding white-box attacks that are effective against our MNIST model.

### The L_infinity MNIST Model
...
### The L_infinity Attack Model

We are interested in adversarial inputs that are derived from the MNIST test set.
Each pixel can be perturbed by at most `epsilon=0.3` from its initial value.
All pixels can be perturbed independently, so this is an l_infinity attack.

### Submitting an L_infinity Attack

To submit an attack for the L_infinity challenge, follow the instructions posted [above](#submitting-an-attack) to generate the adversarial test set.
Then, email the resulting file to mnist.challenge@gmail.com with the subject line "Linf Challenge Submission". 
We will then evaluate the attack in the same way we evaluate L_2 white-box submissions and post the attack on the leaderboard if it is novel or outperforms all current attacks.

As is the case with our L_2 submittions, we strongly encourage you to disclose your attack method, and we would be happy to add a link to your code in our leaderboard.
